...
const isHostedEnv = process.env.NEXT_PUBLIC_NODE_ENV === 'production' ||
                   process.env.NEXT_PUBLIC_USE_HOSTED_URLS === 'true';

// Check if we should use Docker URLs (server-side only)
const isDockerEnv = !isBrowser && (process.env.USE_DOCKER_URLS === 'true' || process.env.USE_DOCKER === 'true');
...
const HOSTED_URLS = {
  ...
  notificationUrl: process.env.NEXT_PUBLIC_NOTIFICATION_URL || 'https://api.stoxify.com/notification',
};
...
export const env = {
  ...
  app: {
    ...
    frontendUrl: process.env.NEXT_PUBLIC_FRONTEND_URL || (isHostedEnv ? 'https://stoxify.com' : 'http://localhost:3000'),
  },
